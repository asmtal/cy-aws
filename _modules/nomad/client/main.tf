...
module "user_data" {
  ...
  files = [
    {
      Path    = "/opt/nomad/config/client.hcl"
      Content = templatefile("${path.module}/files/client.hcl", {NomadClusterId = var.cluster_id})
    },
    {
...
locals {
  tags = {Application = "nomad", Role = "client", NomadClusterId = var.cluster_id}
